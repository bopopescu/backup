...
class Command_analyzer:
    ...
    def set_parameter(self):
        ...
        command_xml = ET.parse(os.path.join(self.pkg_path, 'dataForGPSR/Commands.xml'))# XML ファイルから ElementTree オブジェクトを生成
        command_tree = command_xml.getroot()
        for i in command_tree.findall('command'):#command_treeのcommndと書かれている文字列を探しリストにする
            vbs = []#リスト化
            for j in i.findall('.//verb'):#iが.//verbという文字列を探しリスト化
                vbs.append(j.attrib['name'])#vbsの中にnameのもつディクショナリを追加
            self.verbs.extend(vbs)#vbsの中の文字列がすべてself.verbsに入れる
            self.command_verbs[i.attrib['name']] = vbs#iの要素であるnameのもつ辞書をvbsに追加
        self.verbs = list(set(self.verbs))#self.verbsの語彙を取得しリスト化しself.verbsに入れる
        #verbs.extend(["pick", "look", "come"])
        room_locations = {}
        beacons = []
        placements = []
        with open(os.path.join(self.pkg_path, 'dataForGPSR/location_list_gpsr.yaml')) as yf:#with open すると閉じる必要なし
            location_dict = yaml.load(yf)#locationにyfを書き込む
        self.rooms = location_dict["room"]
        self.category_locations = location_dict["location"]
        for category,data in self.category_locations.items():#itemsカンマで区切り、カンマごとにまとめる
        ...
        for i in name_tree.findall('name'):
            self.names.append(i.text.lower())#iの中の文字列を小文字化する
            if 'gender' in i.attrib:
                self.males.append(i.text.lower())
            else:
                self.females.append(i.text.lower())
        self.names = list(set(self.names))#おなじのを省く
        ...
        todaydate = datetime.date.today()#日付を取得
        today = todaydate.isoformat()#日付を返す
        tomorrowdate = todaydate + datetime.timedelta(days=1)#一日を24時間に変換
    ...
    def callback(self,data):
        ...
        if self.on_flg:
            ...
            if self.mystate == "start":
                ...
                you = 0
                whkey = []
                for whts in self.whattosay.keys():#□:○のときの□を選ぶ処理
